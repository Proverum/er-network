...
async function main() {
    try {
        ...
        const secret = await ca.register({ affiliation: 'municipality3.amtX', enrollmentID: 'clerk1', role: 'client' }, adminIdentity);
        const enrollment = await ca.enroll({ enrollmentID: 'clerk1', enrollmentSecret: secret });
        const userIdentity = X509WalletMixin.createIdentity('Municipality3MSP', enrollment.certificate, enrollment.key.toBytes());
