...
async function main() {
    try {
        ...
        const secret = await ca.register({ affiliation: 'canton2.amtX', enrollmentID: 'clerk1', role: 'client' }, adminIdentity);
        const enrollment = await ca.enroll({ enrollmentID: 'clerk1', enrollmentSecret: secret });
        const userIdentity = X509WalletMixin.createIdentity('Canton2MSP', enrollment.certificate, enrollment.key.toBytes());
        console.log("this is the newly enrolled user identity object", userIdentity);
        await wallet.import('clerk1', userIdentity);
        console.log('Successfully registered and enrolled admin user "clerk1" and imported it into the wallet for the canton 2');
